...
| `Maugham/Canvas/` | The Plan persona's centre column: a SwiftUI `Canvas` draws every node and every region, one real `NSTextView` mounts on the focused scrap off the SAME TextKit stack. Camera is a manual CTM — never `.scaleEffect`, never `NSScrollView.magnification` ([ADR 0026](docs/adr/0026-planning-canvas-rendering.md)). Ground is a Metal shader BENEATH the content. State lives on an `@Observable` `CanvasModel` owned by `ProjectWindow`, so the region inspector in the right-hand column reads the same scene the canvas draws — and mutates it through `mutateFromInspector`, never `mutate` (tripwire 32). Region membership is stored and never derived from geometry (tripwire 31). ⌫ deletes the selected scrap, region or line. **`Promote…` (⌘⇧↩, or the button in any of the inspector's three arms) turns a scrap, region or line into a durable artifact — one command, a snapshot, and the mark it leaves is provenance rather than a link (ADR 0026 §9).** `canvas.md` is content; `.maugham/canvas.json` is derived, at schema 4. **Read `Maugham/Canvas/AREA.md`**. |

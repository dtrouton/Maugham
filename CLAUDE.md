...
- Holds the cross-target substrate: Op/OpKind/Annotation(Deriver)/Materializer/Bootstrap/Reconciler/ParagraphID/ULID/SweepReason/SynthesisSource/Checkpoint(Store), `JSONLAppendStore`, `OpLogStore`, the Fountain parser, `BuildVariant`, the Models (incl. `TypographySettings`/`ProjectTargets`), `Transcriber`, and `Inbox/` (`InboxEntry`, `InboxFileKind`). AppKit/SwiftUI-bound code stays in `Maugham/`. Shared with the future `MaughamPhone` iOS target.

...
- **Op log is the source of truth for manuscripts.** `.md` files on disk are derived. Editing means appending to the per-doc JSONL op log under `.maugham/ops/` and re-rendering. Inline `<!-- ¶id -->` HTML-comment anchors are the join key. See `Maugham/OpLog/` and ADRs 6–8. However, because deletion is not a first-class op, faithful recovery currently also depends on the parsed `.md` on disk plus the `reconcile` heuristics — don't treat a raw op-log replay as a complete manuscript.

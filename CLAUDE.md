...
| `Maugham/Canvas/` | The Plan persona's centre column: a SwiftUI `Canvas` draws every node, one real `NSTextView` mounts on the focused scrap off the SAME TextKit stack. Camera is a manual CTM — never `.scaleEffect`, never `NSScrollView.magnification` ([ADR 0026](docs/adr/0026-planning-canvas-rendering.md)). Ground is a Metal shader BENEATH the content. `canvas.md` is content; `.maugham/canvas.json` is derived. **Read `Maugham/Canvas/AREA.md`**. |

...
# iOS companion (the MaughamPhone target — read MaughamPhone/AREA.md before editing it):
...
- **The two schemes are independent.** `Maugham` builds the Mac app + MaughamCore; `MaughamPhone` builds the iOS app + MaughamCore. A change to MaughamCore (or a cross-module move like the `Deriver` promotion) should be tested against BOTH. Transient simulator "Busy / failed preflight checks" on the iOS test run is a flake — re-run; don't `simctl shutdown all` before a launch (it causes it).
...
- **Triage SourceKit live diagnostics by class — do NOT blanket-ignore them.** Most are *noise*: `No such module 'MaughamCore'/'XCTest'`, `cannot find type X in scope`, `does not conform to Equatable` are decoupling artifacts — SourceKit's index goes stale against the generated `.xcodeproj` + local SPM module until a build warms it, and one unresolved `import` cascades into a wall of secondary false errors. Ignore those; `xcodebuild` is ground truth. **But `the compiler is unable to type-check this expression in reasonable time` is REAL** — that's SourceKit actually running the type-checker and hitting the same ceiling the compiler does. Heed it (see the SwiftUI-body note under `Maugham/Views/`). Blanket-ignoring SourceKit is exactly how a Release-only build failure shipped to CI on the v0.8.0 tag.
...
- **`Packages/MaughamCore` is a local SPM package** (shared substrate using **Apple system frameworks only** — Foundation + CryptoKit for the integrity manifest's SHA-256 — with **no third-party package dependencies**; declared under `project.yml` `packages:`, both app targets depend on it). This keeps it Apple-only by design; a hypothetical Linux port would swap CryptoKit for `apple/swift-crypto`. After editing `Package.swift` or adding package sources, run `./gen.sh`. **WhisperKit** is a *remote* SPM dep on the **Maugham (Mac) target only** — its first resolve fetches from GitHub (needs network); if Xcode's GUI shows "missing package product", quit + reopen the regenerated project (CLI `xcodebuild` resolves fine). `.build/` is gitignored; the package source is **not** (the old blanket `Packages/` ignore was removed).
...
18. **Don't assume an iOS file read is an evicted iCloud placeholder; don't treat `startAccessing==false` as a denial; don't `d_`-prefix a docId that already has one.** The MaughamPhone download gate (`CoordinatedFileIO`/`isLocallyReady`) must treat `.current`/`.downloaded`/nil-status-but-present files as already-readable — only `.notDownloaded` placeholders fetch; assuming all files are evicted broke browsing entirely (2026-05-30 smoke). `startAccessingSecurityScopedResource()==false` means "no scoping needed," not denied — `ProjectsRoot` adopts the folder regardless. Op-log/inbox filenames are `<docId>.<slug>.jsonl` (docId already carries `d_`); a double-prefix lands in a stream the Mac never reads. These + the rest of the iOS gotchas (the `@Observable`-not-`@AppStorage` store pattern, `claudeAccept`-copies-`changes`, no-live-cross-device-updates) are detailed in [`MaughamPhone/AREA.md`](MaughamPhone/AREA.md) — **read it before touching anything under `MaughamPhone/`.**
19. **Anything touched by BOTH the Mac and the phone goes through a contract.** Op-log/inbox filenames, ids, on-disk formats, anchor-stripping, Fountain decisions — the phone must not reimplement what the Mac implements (a stricter local doc-id parser shipped the phone-v0.1.1 "No open annotations" bug). The single sources of truth live in MaughamCore; reach-around tripwires (`TripwirePhoneGrepTest`, `TripwireGrepTests`) fail the build on hand-rolled copies and point you at `docs/superpowers/notes/cross-surface-contracts.md`. Don't add a third implementation of anything in that registry.
...
### `Packages/MaughamCore/` — the shared substrate (Apple system frameworks only)
...
### `MaughamPhone/` — the iOS companion app — see [`MaughamPhone/AREA.md`](MaughamPhone/AREA.md)
- The four-tab iOS app (Capture / Read / Annotations / Settings), shipped + smoke-verified in the iPhone-companion milestone (`memory/project_milestone_iphone_companion_ios.md`, merge `3fff8b5`). Reads/writes a security-scoped-bookmarked iCloud-Drive projects folder; shares MaughamCore; AppKit-bound code never reaches here. **WhisperKit is Mac-only — don't add it to MaughamPhone.**
- **Read AREA.md before editing** — it has the build/test command, the seam-injection pattern (`@Observable` + injected `UserDefaults`/clock/seam, NOT `@AppStorage`; two-layer download seam), and the iOS tripwires the 2026-05-30 smoke surfaced (download gate must treat already-local/non-evicted files as ready; `startAccessing==false` is not a denial; `docId` already carries the `d_` prefix; `claudeAccept` copies `changes` verbatim; no live cross-device updates in v1).
- Phase G (TestFlight pipeline + AppIcon) is **shipped + first release cut** (`phone-v0.1.0`, build 854, 2026-05-31): `scripts/cut-phone-release.sh` (mirrors the Mac cut script, `phone-v*` tags), `.github/workflows/phone-release.yml` (`macos-latest`, version-from-tag, build-number = `git rev-list --count HEAD`, archive → `-exportArchive` with `destination=upload` + ASC API key — **altool-free on purpose**: it's deprecated and silently broken on Xcode 26), and the M+¶ app icon (editable SVG sources + `render.sh` in `scripts/phone-icon/`). One-time Apple setup (6 GitHub secrets, ASC record) is in `docs/release-notes/phone/SETUP.md`. **The dry run IS the integration test** (spec §7.3): cert/profile/API-key/upload + Apple's bundle validation can ONLY be checked by pushing a real signed build to App Store Connect — five on-device bugs surfaced this way, invisible to `xcodebuild test` and to a debug build. So cut throwaway `phone-v0.0.x` tags to prove a fix on device, then cut the real tag (commit-count build numbers never collide); delete throwaways after. The four release-phase bugs: (1) bundle-id casing (see below); (2) `TARGETED_DEVICE_FAMILY` must be `"1"` (iPhone-only) or ASC rejects portrait-only as needing all 4 iPad orientations; (3) `ProjectsRoot.pick` must `startAccessing` BEFORE `makeBookmark` (else the iOS sandbox reports the iCloud folder "doesn't exist" — a debug build worked by luck); (4) screenplay rendering — the Fountain reader must strip `<!-- ¶id -->`/`<!--t-->` anchors via the shared `MarkdownDisplayFilter` like the markdown path (a target-local skip leaked anchors as body text AND broke title-page detection), and `.titlePage` lines must be `hidden` in `FountainStyler` since `FountainSemanticRenderer.titlePageBlock` already renders `script.titlePage` (else the title page double-renders). Original brief: `docs/superpowers/notes/2026-05-30-phase-g-handoff.md`; release session forensics: `docs/superpowers/notes/2026-05-31-phone-testflight-status.md`; milestone: `memory/project_milestone_iphone_companion_phase_g.md`.
- **The phone's release bundle id is `com.Maugham.MaughamPhone` — capital "M", deliberately.** It diverges from the Mac's `com.maugham.Maugham` because the Apple App ID got registered with a capital M, and **Apple namespaces App IDs case-insensitively** so the lowercase form can't be re-registered without deleting the existing App ID + App Store Connect record (an ASC record's bundle id is immutable). Codesign matches the bundle id **case-sensitively**, so the `PRODUCT_BUNDLE_IDENTIFIER` in `project.yml` (Debug `com.Maugham.MaughamPhone.dev` / Release `com.Maugham.MaughamPhone`) and the `provisioningProfiles` key in the workflow's generated `ExportOptions.plist` MUST stay capital-M and in sync, or the export/sign step fails. This is cosmetic-only (bundle ids are internal; Mac↔phone sync is via the shared iCloud folder, not bundle id). If you ever regenerate the App ID, you can switch to lowercase — but then change both project.yml ids + the ExportOptions key together.
...
- **`ScreenplayMode` (AppKit editor) and `MaughamPhone`'s `FountainStyler` (SwiftUI reader) are two independent renderers of the SAME parsed Fountain.** Layout (indents/alignment/uppercase mechanism) legitimately differs per surface; **bold/italic emphasis is contracted** by `ScreenplayEmphasis` (MaughamCore) and enforced by `ScreenplayEmphasisContractTests` in *both* test targets. If you change an element's emphasis on one surface, or add a `ScreenplayElement` case, the contract switch + both tests force you to handle both surfaces. Don't add a third Fountain renderer without wiring it to the contract.
...
### `Maugham/Views/`
- `ProjectWindow.swift` uses extracted `ViewModifier`s (`SessionAndNavigationModifier`, `CollectionPieceModifier`, `CheckpointModifier`, `TopChromeModifier`) to dodge SwiftUI's body type-checker complexity ceiling. **When you hit "the compiler is unable to type-check this expression in reasonable time," extract a ViewModifier** — this is the established pattern. **This is often a Release-ONLY failure:** the Debug build and `xcodebuild test` (Debug) tolerate a heavier `body`, but the Release config's stricter type-check budget fails to compile it — which is how a green local test run shipped a broken Release build to CI on the v0.8.0 tag. The SourceKit "unable to type-check" warning is the early signal (the one SourceKit diagnostic to *heed* — see the Build-flow note). **After any change to `ProjectWindow.body` (or any large SwiftUI `body`), run a local `xcodebuild -project Maugham.xcodeproj -scheme Maugham -configuration Release build CODE_SIGNING_ALLOWED=NO` before tagging a release** — it's the only cheap thing that catches this.

...
| 5 | No `NSPopover` for editor autocomplete | Sizing unreliable, blocked input; abandoned in 3b. `CharacterAutocompleter` is dead code | `Maugham/Editor/AREA.md` |
...
| 7 | No 4th caller to `EditorSurface.applyExternalText` | Exists only for cloud-conflict resolution; a 4th caller is a binding race | `EditorIntegrationHarnessTests` |
| 8 | Use 4-char alphabet-restricted paragraph IDs in tests crossing the `.md` ↔ op log boundary | `ParagraphID.parseComment` rejects ids outside its alphabet; permissive in-memory APIs hide the mismatch until the test hits Bootstrap/Reconciler/RenderFilter | Use `ParagraphID.mint()` or a 4-char literal from `[0-9a-hjkmnp-tv-z]` |
...
| 17 | No single shared JSONL file across devices via iCloud Drive | iCloud's reconciler can't line-merge concurrent appends; the loser is silently dropped as a conflict-twin the loader never opens; inbox `writtenAt` must be monotonic to survive clock skew | `PendingBuffer` partitioned (M1.3); `OpLogStore.opLogFileURLs`/`loadSyncMerged`; ADR 0012 |
...
|---|---|
| `Packages/MaughamCore/` | Shared substrate (Apple frameworks only): Op/OpKind/Annotation/Materializer/Bootstrap/Reconciler/ParagraphID/ULID/SweepReason/SynthesisSource/Checkpoint/JSONLAppendStore/OpLogStore/Fountain parser/BuildVariant/Models/Transcriber/Inbox/MarkdownDisplayFilter. `MarkdownDisplayFilter` is the single source of truth for anchor-stripping (don't add a target-local copy). Cross-module = `public`; the build tells you. |
...
| `Maugham/MCP/` | Single source of truth: `MCPToolCatalog.all`. Implement `MCPTool`, add to catalog; both `MCPToolsListHandler` and `MaughamApp.registerTools` derive from it. Transport = live-only Unix socket (ADR 0003). **43 tools** (see AREA.md for full list). Tools fail loudly on unknown ids. **Read `Maugham/MCP/AREA.md`**. |
...
| `Maugham/Views/` | `ProjectWindow.swift` uses extracted `ViewModifier`s to dodge SwiftUI's body type-checker ceiling — extract more when you hit the limit. BinderSegment conditional cases auto-coerce to `.manuscript` when their condition disappears. Right-pane mode-swap (⌘⌥1/2/3) is the established pattern (ADR 0005). |
| `Maugham/Models/` | `ProjectType` is polymorphic; Collection references are Mac-local. Manifest `modified` must round-trip through ISO8601 with whole-second rounding. Schema evolution: add `unknown` enum cases + custom decoder for non-optional fields (ADR 0014). |

## Inbox + transcription (`Maugham/Stores/` — iPhone-companion Mac side)

`InboxStore` owns `.maugham/inbox/` (per-device `inbox.<slug>.jsonl`, monotonic `writtenAt`). `InboxPane` is ⌘⌥6. `InboxTranscriptionWorker` runs an injected `Transcriber`; production is `WhisperKitTranscriber` (Apple-Silicon only). MCP scope is **read + promote only** (no add/trash from MCP).
...
- Onboarding affordance for Annotations pane vs History pane is missing — sibling segments with opposite affordances (action surface vs read-only forensic log). A tooltip or empty-state hint would reduce confusion.

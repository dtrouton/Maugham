...
- **Op log is the source of truth for manuscripts.** `.md` files on disk are derived. Editing means appending to the per-doc JSONL op log under `.maugham/ops/` and re-rendering. Inline `<!-- ¶id -->` HTML-comment anchors are the join key. See `Maugham/OpLog/` and ADRs 6–8. The op log is the source of truth, full stop, with **`sequence` authoritative** — a deleted paragraph's id drops out of `sequence` even though its key lingers in the derived `paragraphs` map, so materialize/walk by `sequence`, never raw `paragraphs`. **External `.md` edits are not honored**: editing is through Maugham, cross-device sync flows through the op-log merge (ADR 0012, not `.md` reconcile), and an outside `.md` mutation — including a deletion — is discarded on re-materialize. The one residual `.md`-on-load dependence is a *legacy* recovery detail (sequence-less bursts predating the always-capture-sequence fix; see the `Deriver` comment), not a license to treat the `.md` as authoritative. (2026-06-09 decision: "blow away changes to the .md outside of Maugham" is intended; deletion-ingest rejected.)
...
19. **Anything touched by BOTH the Mac and the phone goes through a contract.** Op-log/inbox filenames, ids, on-disk formats, anchor-stripping, Fountain decisions — the phone must not reimplement what the Mac implements (a stricter local doc-id parser shipped the phone-v0.1.1 "No open annotations" bug). The single sources of truth live in MaughamCore; the reach-around grep tripwires (`TripwirePhoneGrepTest`, `TripwireGrepTests`) are recurrence-trippers — they catch the *known* bad spelling so the same mistake can't silently return — not a complete fence. The real safety net is the round-trip integration tests that exercise the Mac's own readers. Both tripwires point you at `docs/superpowers/notes/cross-surface-contracts.md`. Don't add a third implementation of anything in that registry.

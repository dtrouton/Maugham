...
| `OpLog/Document+Annotations` (+`AnnotationDeriver`, `AnnotationInverse`) | 56 | 34% | 19 / 0 | `28-annotations-reconciliation.md` |

The three MaughamCore rows are the 148 reconciled claims out of the ledger's 169; the app-layer rows are 155 further claims in their own files. **324 claims in the experiment, 303 reconciled.** The app layer stands at **82 complies / 0 violates** (MaughamCore's pure modules ran 31:1 — the inversion result).

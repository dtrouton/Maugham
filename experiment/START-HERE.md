...
| `OpLog/Document+Rewind` (+`RewindUndo`, `Deriver+Rewind`) | 34 | 59% | 19 / 1 | `24-rewind-reconciliation.md` |
| `OpLog/Document+Annotations` (+`AnnotationDeriver`, `AnnotationInverse`) | 53 | 28% | 8 / 7 | `28-annotations-reconciliation.md` |

The three MaughamCore rows are the 148 reconciled claims out of the ledger's 169; the app-layer rows are 138 further claims in their own files. **307 claims in the experiment, 286 reconciled.** The app layer stands at **40 complies / 19 violates** (MaughamCore's pure modules ran 31:1 — the inversion result).

...
- **Organise and navigate.** The Inspector holds per-piece synopsis, tags, and word targets, plus a status derived from where the piece stands on each of its review passes. The Outline pane (corkboard or table view) gives a birds-eye view of your project. `[[wiki-links]]` connect related pieces; Research lives in its own browser pane beside the draft.
...
- **Review passes.** Name your own editing passes — Structural, Line, Copyedit, Proof, to start — and track where every piece stands with a board (Review persona), a triaged queue, and ⌘R rounds that report the distance travelled since the last one. Each pass has a named editor voice (Perkins, Lish, Gould, Argus) with its own brief, and the loop — check, findings, dispose, next round — runs from either Author's own wet-ink view or Review's round cockpit.

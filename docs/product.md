...
**Planning.** The Plan persona (⌘1) puts a **planning canvas** where the editor usually sits — a freeform spatial surface for thinking before anything has firmed up. Cards are typed straight onto it, dragged, resized, and zoomed; a region drawn around what belongs together moves as one and can be bound to the piece it is material for; an untyped line joins two cards that have something to do with each other. Research rows from the binder, photographs from the Finder or a web page, and captures from the phone's inbox all drop onto it directly. It is deliberately scratch — most of what lands there never becomes anything — and **promotion** (⌘⇧↩) is the one explicit step that makes a piece of it durable: a card, a region, a line, or a photograph the canvas owns becomes a research note, a palette card, a line in the craft intent, or a `[[wiki-link]]`. Promotion takes a *copy* and leaves a mark saying where the words went, so the card keeps its own. Card text is plain `canvas.md` at the project root and photographs sit beside it in `canvas_assets/`; where things sit on the surface is derived layout, and deleting that never costs a word.

**Research.** A research browser lives beside the draft — notes, images, PDFs — plus a *sensory palette*: subject-keyed cards (a character, a place) holding color swatches, reference images, and sense-tagged notes, paired with an optional **craft intent**: a right-pane statement (⌘⌥N) stating what the story needs sensorially, scoped to the project or to a single chapter, carried as an op-logged document so it has history, undo and cross-device merge like the manuscript. Its sibling pane, **Visual Language** (⌘⌥V), holds how the book *looks* — prose with pictures dropped or pasted into it.

**Working with Claude.** A bundled local MCP server exposes the whole project to Claude Desktop. Claude can read everything — binder, manuscript, research, wiki-graph, tasks, palette, planning canvas — and can *propose*: comments, queries, craft notes, and suggested changes, all anchored to specific paragraphs. It can also add cards to the **planning canvas**, which is the one surface it writes to besides research: the case built for is a page the writer filled by hand and photographed, and Claude cannot choose where a card lands — every batch arrives in one labelled region beside the page it was read off, drawn perfectly straight where the writer's cards lean, and one ⌘Z takes the whole batch back. Proposals land in an Annotations pane where the writer accepts, rejects, or archives each one; the MCP layer has no tools that modify manuscript text. Claude can also translate the book into another language (a parallel, paragraph-keyed layer — the source manuscript never changes), compile it (below), and answer help questions from the same guide the app ships.

**Getting read.** ⌘R asks Claude to read the open piece, and *which* read it is belongs to the persona you are standing in. In **Author** it is a **check** — what changed since the last one, read by the coach (Le Guin, a standing relationship rather than a rung on any ladder) or by a **first reader** the writer names and describes for this project, who reports where the dream broke, what she now believes, where she dragged and where she was lost, and never proposes a fix. In **Review** it is a **round** — the piece read whole by the editor of the pass it sits in, filed and numbered in that lane, leading with what has changed since the last round. ⌘⇧R is the cold read under each loop's own name: *Reread* in Author, *Fresh Eyes* in Review. A read ends in a letter written as prose rather than a list, and every note it leaves lands in the same annotation membrane as anything else Claude proposes — the writer disposes. Claude reads who the writer is writing toward through `read_first_reader`, and writes it never.

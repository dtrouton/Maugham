...
The right column has seven modes: Inspector, Research, and Outline (covered on this page), plus Tasks, Annotations, History, and Palette (each covered elsewhere — see the pointers at the bottom of this page). Switch with the numbered/lettered shortcuts below or the small picker at the top of the pane.
...
It also has a **History** mode (⌘⌥4) — a read-only timeline of edits, annotations, and checkpoints, with a scrubber for time-travel — and a **Palette** mode (⌘⌥7) — a card for the sensory palette wall, see [The Sense Pass](sense-pass.md).

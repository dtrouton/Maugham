...
**A clean check says so plainly.** A conformance section where every clause holds still renders — that's the good outcome, not an empty pane — and a check with no strain at all to raise reads *"Nothing to flag."*, not silence and not a green checkmark standing in for an answer. A check that raised a continuity question or a reader's report instead never claims there's nothing to flag while **This check** still has rows in it — the header only says so once you've settled them (or once the check genuinely found nothing).

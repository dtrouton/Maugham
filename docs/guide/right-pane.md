...
It reads as a report: **Conformance** — every clause you've declared, quoted back in your own words, each *holds*, *strains*, or untouched by this draft, with an **Open Intent** button in the section header. No strain ever shows a paragraph id — each carries the words that paragraph said, as a chip you click to jump there. *(A continuity question and the reader's report used to be two more sections here — they arrive as notes in your queue instead now; see below.)*

...
**Failing-loudly:** If you try to compile a language edition but no source publication exists yet, it refuses with a clear message: *"compile the source edition first or pass version."* And if a compile of the same version, language and format is already running, a second one refuses straight away — *"Publication v1.0 (source, pdf) is already compiling; wait for it to finish."* — instead of racing it into a half-written file. Poll the one in flight with `compile_status`, or compile a different format or language in the meantime; different editions still compile side by side.

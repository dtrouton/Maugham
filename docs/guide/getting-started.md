...
**Selecting something in the tree can dim the board too.** Select a chapter or a group and every card and region not bound to it dims, so what's relevant to that selection stands out. Select a research note or palette card instead and the canvas looks for its card the same way: already on the board, and that one card lights up while the rest dims; not there yet, and the whole board dims with a line in the middle — *"This item isn't on this canvas yet. Drag its row from the tree to place it."* — that clears the moment you drag the row over. Arriving at a research subject this way, or via Claude's **Show**, brings its card into view if the camera isn't already looking at it; reopening the project on a research subject you'd left selected doesn't move the camera again, so your view stays exactly where you left it.

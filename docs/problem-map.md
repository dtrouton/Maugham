...
- ✓ **Think before it's a book** — spread half-formed pieces out somewhere with no schema and no commitment, and shove them around until the shape appears. *(planning canvas in the Plan persona: scraps typed straight onto a spatial surface, moved, resized, zoomed, deleted; regions drawn around what belongs together, moved as a unit, collapsed when the canvas gets crowded, and associated with the piece they are material for; a card lives in one region and can be cited in any number of others, so nothing has to be filed anywhere before it is understood; lines drawn between two cards that have something to do with each other, named or left unsaid, and untyped by design; and **promotion**, the step that makes the scratch pay off — one explicit command (⌘⇧↩) that turns a card into a research note, a palette card or a craft intent, a region into a research note or a palette card, and a line into a `[[wiki-link]]` once both its cards have been promoted, always previewed before anything is written and always a copy, so the card keeps its words. A card carries the same piece association as a region, and **where a promotion lands follows it** — the card's own piece, else the region it lives in, else the project's research — and **Claude can put things on it** — the case built for is a page written by hand and photographed, read and turned into cards in one labelled region beside the page itself, marked as Claude's by being drawn perfectly straight where the writer's cards lean, takeable back in one ⌘Z, and durable only through the same promotion. **And four routes now put something on it** — a research row dragged out of the binder, a photograph dropped from the Finder or a web page, a capture sent straight from the Inbox by drag or by command, and Claude's own batches — where a card standing for something shows its real title, its kind glyph and, when it is a picture, the picture itself, sized from that picture's own shape. A photograph the canvas owns promotes like anything else, to a research asset or onto a palette card, and a region carries the pictures in it onto the card it produces. `⌘\` folds both side columns away and gives the canvas the window)*
...
- ✓ **Ask for feedback at the tempo I'm working at** — a quick read on what I wrote this morning is not the same act as a copyeditor's pass over the finished chapter, and one key should not silently do both. *(two loops: ⌘R in Author is a **check** over what changed since its own last one, read by the coach or your own first reader, unstamped and un-numbered; ⌘R in Review is a **round**, read whole by the piece's stage editor and filed and numbered in that pass's lane. The persona is what says which, and the File menu names each one's verb)*
- ✓ **Write toward one particular reader, and hear back from her** — the person whose eyebrow you can see going up, named and described rather than imagined. *(the first reader: one per project, named in Project Settings and described in her own statement, chosen from the Diagnostics header's picker; she reports where the dream broke, what she now believes, where she dragged and where she was lost — never craft vocabulary and never a fix — and her letter is the reader's short form. Standing instructions to her are her statement's own `## Rulings`, and answering one of her notes files there in the same act as the reply)*
...
- ✓ **Know that every word in my manuscript is mine or somebody I named** — a change appended by something other than Maugham, or by a machine I never admitted, is held and told about rather than quietly becoming my prose. *(signed op log P1/P1b/P2: hash-chained appends, per-actor Secure Enclave keys, a per-project registry of admitted devices, one admission panel per machine, People & Devices, History's banners and dated entries)*
- ~ **Let a trusted reader into the book on my terms** — the registry can admit, revoke and retire a device, and two Macs can merge, but that is provenance between my own machines. A reader with a role of their own, checked per op kind, is P3. *(partial — labels only; no person key, no roles, hard revocation is still the iCloud share's)*

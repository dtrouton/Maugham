...
- • **Backup & integrity** (design approved 2026-06-07, pre-implementation) — an **integrity primitive** (Merkle manifest, surface + quarantine the today-silent op-log parse-skip, manifest shadow, checkpoint-pointer / set / derive-and-compare checks, a "Verify project" action) + a **filesystem-only backup system** (global app-level destinations as security-scoped folder bookmarks → any local/cloud-synced/external folder; safe-by-default, every project keyed by minted `ProjectManifest.id`; full-local / essential-remote scope; atomic clone-or-copy generations; deep-local / shallow-remote retention; test-restore verification against `Checkpoint.manuscriptWordCount`; per-destination status) + **restore** (restore-beside never-overwrite; single-doc-via-ops; auto-bisect-to-good). Backup is the recovery arm of the integrity primitive; complements (does not replace) Time Machine. Encryption deferred to external (provider at-rest / encrypted disk). Spec: `docs/superpowers/specs/2026-06-07-backup-and-integrity-design.md`. *Replaces the old "iCloud version-history surfacing / Time Machine note / cross-session undo" sketch.*
...
- • **Op-log compaction (Automerge/CRDT-style)** — snapshot derived state + truncate ancient ops into a compacted base, keeping recent ops for rewind; bounds unbounded op-log growth and shrinks backups/derive time. Deep rewind beyond the compaction horizon is preserved in pre-compaction **Backup & integrity** generations — the two are designed to interlock. Surfaced during the backup brainstorm (2026-06-07); pairs with the performance pass.

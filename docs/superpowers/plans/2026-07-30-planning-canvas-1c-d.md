...
- **Disable experiment (substituted 2026-07-31, stricter than the original):** drop the `cachedHeight:` argument on the created node and the membership tests go red while every creation test stays green. The original — move the join above `rebuildLayouts()` — falsified an *ordering* that merely happened to produce a measured node; this falsifies *"a dropped node lands measured"* directly. **Original and the membership tests go red while every creation test stays green. That is the exact defect the ordering comment exists for.

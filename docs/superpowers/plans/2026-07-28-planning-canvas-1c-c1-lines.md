...
**And that contract is a trap as well as a foundation, so the route must read the selection AS OF THE PRESS.** *(Added 2026-07-28 during Task 4; the first draft of this plan named the rule and not the mechanism, and the obvious mechanism breaks it.)* `handleClick` reassigns `model.selection` to whatever is under the pointer, and it runs **first** — so a `.began` arm that reads `model.selection` live sees the card the press just selected, and the very first press on an unselected card's mark position draws a line. That is not a cosmetic slip: it puts a 14 pt patch on the right edge of **every** unselected card that refuses to move it. Take one `@State selectionWhenPressed`, written at the head of `handleClick` before anything can move the selection and read once in `handleDrag(.began)`. A stale value is harmless, because the route also requires the selected card to be the one under the pointer.

**Require `topmostNode(at:)` to BE the selected card, not merely to exist.** A card drawn in front hides the selected card's mark, and `begin` resolves the source with `topmostNode` — so without that term the writer presses a card they can see and gets a line out of one they cannot.
...
**`.ended` must CLOSE the line above `end()`, not inside the chain — the snippet below is wrong and Task 4 corrected it.** *(Corrected 2026-07-28 during Task 4.)* A swept rect survives being read into a `let` because it is a value; **a source card does not.** `endLine` resolves its source off the live mode, and by the time the `if/else if` chain runs, `let flick = interaction.end()` has already set the mode to `.idle` — so an `endLine` call inside the chain returns nil on every drag, silently, and no line is ever created. Close the line beside the two values already read above `end()` for exactly this reason, and let the chain carry only the selection assignment (`else if wasDrawingLine { if let mintedLine { model.selection = .line(mintedLine) } }`). A side benefit falls out: `end()` then runs on an idle mode, so a line drag reports no flick **by construction** rather than by the `guard case .moving` continuing to hold.

With that correction, `.ended` branches in the same `if/else if` chain:
...
**Lift TWO projections onto `CanvasScene`, for one reason.** Both are scene knowledge parked somewhere else, and each cost a layering inversion when a second caller needed it — the pattern is the point, so do them together:

- **`drawnLines`** — read by both `CanvasRenderer.lineGeometry` and `CanvasLineHit.line(at:in:)`.
- **`selectionTarget(at:in:)`** — currently a `static func` on `CanvasView`, now called from `CanvasInteraction.begin` so the click and the drag share one precedence (Task 5's fix). That is the right sharing and the wrong direction: it points interaction → view. Moving it to `CanvasScene` keeps the single spelling and puts every caller below it. Task 5 made hit testing walk `lineGeometry` so the `!scene.isHidden(_:)` filter has one spelling, which was the right call over duplicating it — but it points a pure geometry enum at the draw layer, and it leaves `public CanvasLineHit` resting on internal `CanvasRenderer.LineGeometry`, which forecloses ever moving it to MaughamCore. The projection is **scene** knowledge parked in the renderer. Keep the one spelling; put the dependency the right way round. *(Task 5's review; deliberately deferred to here because this task is already opening that section.)*

**Resolve `CanvasScene.lines(touching:)` in this task, one way or the other.** It has **zero** production callers after Task 6 (censused twice, confirmed both times) — `CanvasScene.remove` scrubs over `linesByID.keys` itself. **This task's own accessibility work is its last chance**: naming how many lines touch a node is exactly what it is for. Use it there, or **delete it**. What must not happen is shipping it: this area has produced three functions that were built, exercised and unreachable, every one found by counting callers rather than by a test, and shipping a fourth while holding the census that names it would be the worst of both.

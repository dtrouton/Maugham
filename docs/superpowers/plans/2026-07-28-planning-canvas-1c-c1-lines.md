...
**Lift the drawable-lines projection onto `CanvasScene`** — a `drawnLines` (or similarly named) property read by both `CanvasRenderer.lineGeometry` and `CanvasLineHit.line(at:in:)`. Task 5 made hit testing walk `lineGeometry` so the `!scene.isHidden(_:)` filter has one spelling, which was the right call over duplicating it — but it points a pure geometry enum at the draw layer, and it leaves `public CanvasLineHit` resting on internal `CanvasRenderer.LineGeometry`, which forecloses ever moving it to MaughamCore. The projection is **scene** knowledge parked in the renderer. Keep the one spelling; put the dependency the right way round. *(Task 5's review; deliberately deferred to here because this task is already opening that section.)*

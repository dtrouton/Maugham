...
It does not happen today, because `OpLogStore` is `@MainActor` and although `sealTailIfNeeded` is `async`, its body contains **no `await`** — so it runs to completion without a suspension point and `append` cannot interleave.

> **Corrected 2026-08-01, during Task 5.** This section originally claimed the hazard was undocumented. That is false: it is documented at `OpLogStore.swift:167–175`, including the exact failure mode. The claim was made without reading the lines immediately above the function, and the correction is recorded rather than silently edited because the spike's whole subject is unwritten assumptions and it would be dishonest to quietly improve its motivating example.
>
> **The finding survives in sharper form.** The existing note argues: *"the seal runs on the same MainActor as every append, so the interleaving requires two same-variant app instances on one Mac, which the app's single-instance model rules out."* That inference does not hold. Same-actor does **not** make an `async` body atomic — actor isolation is released at every `await`. Same-actor is sufficient only given the absence of a suspension point, which the note never states. So the load-bearing fact is undocumented even though the hazard is documented, and the failure mode is worse than a plain omission: a future `await` leaves the note still reading as a valid justification, so a maintainer who checks the stated premise (still single-instance? yes) concludes the gap is closed while it stands open.

**The safety of the seal rests on the absence of an `await` in a fifty-line body — a property of those lines, not of the concurrency model the note appeals to.** No test pins it, and the obvious future edit (making compression or coordination async) reopens the window with no diagnostic. This is the archetype of what the spike is looking for: not an undocumented hazard, but a documented one whose stated reason is insufficient for its conclusion.

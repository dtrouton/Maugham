...
**An update re-records.** Rewriting a region's note rebuilds its contributors: cards that have left the region since stop claiming it, and cards that joined start. The note is written from the current members, so the record follows the same set. ~~**The record is single-valued and the most recent contribution wins**~~ **(2026-08-09 amendment, RULING-51: a contribution record is a FACT, and Maugham holds every one.** A card whose words genuinely sit in two artifacts records both — `contributedToItemIDs`, ordered by when each contribution happened — and the rewrite-guard below protects every artifact the card fed, not just the latest. The original cost argument — a growing list of danglable ids — was put to Denver beside the guard's mechanics and ruled against: both placements were the writer's deliberate acts, and latest-wins re-opened the one-card-rewrites-a-six-card-note misoffer for every earlier note. A rewrite removes and re-records its own artifact's fact only. The MARK stays a single bookmark, latest wins — that is RULING-47's split, not this rule's.)
...
**Amendment, 2026-07-30 (Denver, re-deriving 1C-c3; built in that slice).** Five rulings settle the shape this section left open, and they are the design rather than a plan's reading of it. Decisions of record are in [ADR 0026 §10](../../adr/0026-planning-canvas-rendering.md).

1. **Two tools, and no id or position on the wire.** `list_canvas` reads the canvas whole; `add_canvas_scraps` adds cards. Claude can name **no node id, no region id and no coordinate**. Lines are drawn by `connect`, which indexes the call's **own `scraps` array** — so Claude can draw the arrows it read off a page and can touch nothing the writer made. This closes §10's write-surface question, and it is how constraint 2 above stops depending on Claude's cooperation: there is no parameter with which to place a derived scrap loose.
...
3. **Two visible signals, and the primary one is the TILT** — *amended mid-slice, and the original ruling is kept because the reasoning matters.* As written, this ruling was colour alone: Claude's cards papered a cooler value and Claude's lines stroked one, reusing one visual language rather than inventing a second. Measurement then found the light-mode card tint capped at 5.7 levels of 255 between two large adjacent white fields. The answer was not a bigger tint but a signal the surface already had: **the canvas says "a hand placed this" by leaning, so Claude's things are drawn at exactly 0°**, human cards *and now regions* lean by at least a minimum, and true zero is reserved. Colour ships too, corrected (hold blue, lower red and green — a tint, not a dim); the writer deletes whichever does no work at the smoke. Regions had no irregularity before this and gained it, because a region is the primitive every call creates and it carries no tint at all.
4. **The mark stays when the writer edits the card.** Provenance records an act, exactly as a promotion mark does (§6.1). Structural consequence: **the scene gains no verb for clearing an author** — nothing to write, so nothing to get wrong, and provenance cannot come to depend on how much of a card the writer retyped.
5. **Every add lands in a labelled region; a source is optional.** With a `source_item_id` the region also holds an item node for the page and puts it at the **top** of the column, which is constraint 2 satisfied by reading order. Without one the region is still there and still labelled — nothing Claude adds is ever loose. An **existing** page is never moved into the new region (that would be the geometry→membership transition §4.2 refuses); it is *cited* where it lives.

**Claude's source must already be a research item, and the asymmetry with §8A.4 is deliberate.** `source_item_id` is a **research item id** — not an inbox entry id — because `read_document` is the catalogue's only image reader, so a photograph Claude can *see* is one that has already been promoted. The order for a photographed page is therefore `list_inbox` → `promote_inbox_entry` → `read_document` → `add_canvas_scraps(source_item_id:)`, and the refusal on a wrong id names `promote_inbox_entry` while saying why it might apply. **§8A.4's route has no such precondition** — the writer drags a capture straight onto the canvas — and that is the point of the asymmetry rather than an inconsistency: the writer can already see their own photograph. If Claude is ever to read a capture in place, the missing piece is an image response on `read_inbox_entry` (today: text, transcript, kind and asset *filename* only).

**What 1C-c3 did not make visible.** The page draws as a dashed placeholder carrying its reference id until 1C-d gives item nodes a thumbnail. So the corollary above is served **structurally** — one region, membership recorded, origin recoverable, announced to VoiceOver — and **checkable by looking only from 1C-d**. Recorded so it is not read as done.
...
- ~~**The shape of the MCP canvas write surface** (§8A.2) — one tool or several, and how a region is addressed when Claude groups a photo with what it read.~~ **RESOLVED 2026-07-30 (1C-c3): two tools — `list_canvas` to read the canvas whole and `add_canvas_scraps` to add cards — and a region is addressed *not at all*.** The write signature can express no region id, no node id and no position, so the grouping is the canvas's decision: every call mints one labelled region and puts the page at the top of it. `connect` indexes the call's own `scraps` array, which is how lines are drawn without ids. That is not a convenience — it is this section's own corollary made structural, since there is no parameter with which to place a derived scrap where its origin is unrecoverable. See the §8A.2 amendment below and ADR 0026 decision 10.

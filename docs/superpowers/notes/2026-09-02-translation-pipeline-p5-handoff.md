...
All four gates ran with the screen LOCKED (`CGSSessionScreenIsLocked=Yes`) and no other session's `xcodebuild` on the machine; every skip is the standard locked-screen set (`TreeTravelRowMountingTests` / `PaletteWallDoorTests` click tests, whose count flips between runs, the four `TypingLatencyProbeTests` probes, `EditorCoordinatorCycleTests`' scene-nav, `UpdateInstallerTests`' cross-volume swap, `OpLogGrowthBaselineTests` ×2) — none on this branch.

- **Gate 1 at `9839689e`** (all eight tasks, concurrent with nothing): `maugham-full-20260902-143819.xcresult` — **TEST FAILED, exactly one failure**: `MCPProtocolHandlersTests.test_toolsList_returnsAllExpectedTools`, a hardcoded expected-name `Set` pinning the production catalogue that no task's pre-commit list ran. Fixed in `b5a1a05b` (the two `propose_` names added; sweep found no other hardcoded catalogue list).
- **Gate 2 at `b5a1a05b`** (concurrent with the whole-branch review): `maugham-full-20260902-144502.xcresult` — Passed, 7593 / **7580 passed / 0 failed** / 13 skipped.
- **Gate 3 at `2de687f6`** (after the fix wave): `maugham-full-20260902-151051.xcresult` — Passed, 7594 / **7582 passed / 0 failed** / 12 skipped.
- **Gate 4 at `79d81eac`** (the merge head, after the fix wave's one-line residual): `maugham-full-20260902-151450.xcresult` — Passed, 7594 / **7583 passed / 0 failed** / 11 skipped.
- No `ProjectWindow.body` change on this branch, so no Release build was owed.
- **Whole-branch review (opus) over `aa028ca6..b5a1a05b`:** 0 Critical, 1 Important (a throw after Adopt's essay write — in the glossary loop, the `after` read or `discard` — left the essay and some rulings in the statement with no undo step and the slot still pending, so a retry duplicated the glossary entries; generalising a Task 3 deferred minor), 4 Minor. One fix wave (`2de687f6`): Adopt is now all-or-nothing (a mid-tail failure restores `before` whole through `mutateStatementText` and rethrows; pinned by a black-box test that makes the proposals directory read-only so `discard` throws), the notice line maps a non-`Failure` error through `description`/`localizedDescription`, the gate's type doc no longer names `rollbackUnusedStatement`, and "dot" became "mark" for the ⌘⌥V badge in the roadmap and guide. The scoped re-review (opus) found the notice fix had landed at the `adopt` site only; the `discard` site followed in `79d81eac`. Its triage of the ledger's deferred minors: only the `discard`-tail item had to be fixed before merge; everything else in "Carried forward" above may stay carried.
...
- **Any catalogue change needs `MCPProtocolHandlersTests` in its pre-commit list.** Task 2's list carried `DocSyncTests`, `CompilerAllowlistTests` and `MCPCatalogConsistencyTests` and was green; the hardcoded name `Set` in `test_toolsList_returnsAllExpectedTools` was found only by gate 1. The tool count moves rarely, which is exactly why the suite that pins it is forgotten.
- **A resumed implementer can time out with no work done.** The first fix-wave dispatch went to Task 3's implementer (context intact) and died on a request timeout before its first edit; the tree was clean, so a fresh agent took the same brief. Check `git status` before assuming a failed resume left anything behind.
- **A fix report's "both sites" is a claim, not evidence.** The fix wave changed one of two identical `proposalNotice` lines and reported both; the scoped re-review's diffstat (`2 +-`) caught it. Ask fixers to paste the `xcodebuild` summary and read the stat line, not the prose.
- Three of the plan's own code snippets were wrong in ways only a test run could show — `.iso8601` truncating `proposedAt` so a staged proposal never equalled its re-read, `StatementEssay.recomposed` applied to a visual language that merely looked like it carried rulings, and `catch let x as ConcreteType` over a typed-throws call crashing this toolchain's SILGen. Each implementer fixed the snippet, said so, and the reviewer judged the deviation on its merits — the process worked, but a plan that spells code is a plan that ships its author's untested assumptions.

...
1. **Shipped** on branch `claude/signed-op-log-perf-step-2026-09-09`: a byte-level `prev(ofLine:)` and a root-recording, load-pruning `OpLogDeviceState` (`296ada6c`); `ISO8601Fast`, a hand parser for the two ISO 8601 shapes this app writes with the formatter and epoch fallbacks behind it (`a9197b87`, corrected by `440ae99d`); and the project stream rotating at the same 512 KB as every other tail, at the project-open sweep (`4543b1a4`).
2. **Refused — item 5's "coalesce persists to one per `sealChain`."** The head is remembered BEFORE the batch is written by design (ADR 0032 §3): a crash between them leaves a head no line hashes to, which the load adopts, where the other order leaves the writer's own last op after the remembered head and quarantines it. Deferring the persist would turn a crash into a set-aside of the writer's own words. It is already one persist per batch. Item 5 shipped as prune-only plus a pinned persist count.
3. **Refused — item 2's 1-to-9-digit fraction interface.** The fast path takes ONLY `YYYY-MM-DDTHH:MM:SSZ` and exactly three fraction digits, because the `ISO8601DateFormatter` it stands in front of TRUNCATES fractions past three digits; a fast path accepting more would answer differently from the reader it replaces. Every other shape falls through. The review caught that divergence and `440ae99d` closed it. The ENCODING side is byte-untouched.
4. **The numbers**, quiet machine, best of three, before (`8e9aabf7`) → after (`4543b1a4`): cold verified load 5934.2 → 514.6 ms (−91.3%); warm 5918.7 → 503.1 ms; parse-only control 5832.0 → 421.5 ms (−92.8%); chain walk over the live tail 48.9 → 33.9 ms (−30.7%); seven sidecar reads and signature checks 1.7 → 0.8 ms. The end-to-end fall is the date parse, not the chain — a disable experiment with the one wiring line commented out read 5817.6 ms again.
5. **Decisions #1, #3, #4 and #5 of the P1 handoff are closed** and answered in place there, with the budget restated as the chain's own cost (under 20 ms realistic, under 100 ms on the fixture's oversized tail) and both figures holding. One thing is left open for Denver: a project root that is UNREACHABLE at launch (a detached external drive) is pruned like a deleted one.

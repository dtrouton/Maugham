...
| Fountain scene numbers (`#4A#`) | 2A | `FountainLine.sceneNumber: String?` (MaughamCore), populated by `FountainTokenizer.extractSceneNumber(from:)` off a `.sceneHeading` line, plus a `.sceneNumber`-kind `FountainInlineSpan` over the marker range. Mac editor (`ScreenplayMode`) and phone reader (`FountainSemanticRenderer`) both fade the `#…#` marker dim — same treatment as any other forced-syntax marker, contracted to agree. Publish (`FountainNodeMapper` → `ProjectAST.sceneHeading(_:sceneNumber:)`) right-aligns it: LaTeX via `\scenenumber` (`LaTeXBodyEmitter`), XHTML via a `scene-number` span (`XHTMLBodyEmitter`). | No dedicated cross-surface contract test yet (each surface's own tests — `FountainSceneNumberTests`, `FountainNodeMapperTests`, `LaTeXBodyEmitterTests`, `XHTMLBodyEmitterTests` — pin their own behavior; nothing yet asserts Mac-fade ≡ phone-fade beyond both hand-matching the same dim-opacity constant). | ✅ |

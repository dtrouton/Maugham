...
///   - filename-safe (the `device` string is `DeviceIdentity.deviceId`,
///     `<actor>-<16 hex>` off that actor's key fingerprint — and, in op logs
///     written before the signed op log, a hostname or `phone:<uuid>`, which
///     may carry dots, colons and spaces; both must sanitize);

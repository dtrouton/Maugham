...
enum ProposableStatement: Equatable, Hashable, Codable, Sendable {
    ...
    /// The reverse map. `nil` for a kind nothing may propose — intent, the
    /// lessons ledger (the writer's own record; Claude reads it through
    /// `read_lessons` and never writes it, editorial letter P2), and a newer
    /// build's `.unknown`.

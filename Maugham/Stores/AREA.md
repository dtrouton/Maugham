...
**`appendToStatement(_:to:session:)` is where anything OUTSIDE the pane puts words into a statement** *(M1A Task 12 review; it was `PromotionPerformer.append(_:to:)`, and is shared now that visual language's picture ingest reaches it too — read the call sites for who does)*. Its destination is named by **statement**, never by "whatever the pane is showing": both callers suspend — a promotion awaits its snapshot, an ingest awaits a manifest write and a file copy — and the writer can change panes in that window with a keystroke. The live `Document` first through `openStatementDocument(id:)`, then the open gate, then a transient `Document.load`/write/`close`; the lookup-plus-write does not suspend, so a pane cannot close its `Document` between them. `session` is the caller's, so the ops carry who wrote them. **This is the seam that takes `lockStatementOpen`** — the ingest and the promotion are both callers of it rather than takers of the gate themselves.

...
A **statement** — the writer's intent, or the book's visual language — is a `Statement` entry in `ProjectManifest.statements` whose content is an ordinary `Document` living **in the open at the project root** (`intent.md`, `intent/<slug>.md`, `visual-language.md`; spec §2.2). It is the artifact that replaces craft intent. **Canvas promotion moved onto it in Task 7**; `ProjectStore+CraftIntent.swift` is still present and still has consumers (`read_craft_intent`, the piece inspector's affordance) until Task 8 moves the rest.

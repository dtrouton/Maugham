...
| `.maugham/ops/__project__.jsonl` | `ProjectStore+Tasks` | Reserved synthetic doc id for project-scope pane-created tasks (milestone-tasks). The op log is real but no manuscript backs it. **Do not allocate a real document with id `__project__`.** Since 2026-09-09 the stream rotates into `.mzseg` segments beside it at the project-open sweep, on the same 512 KB threshold as every other tail — it is named explicitly there because the manuscript-id reader excludes it |
...
| `.maugham/statements/proposals/<key>.json` | `StatementProposalStore` | A staged proposal for a brief (edition-brief-<tag>) or the visual language; derived; one pending slot per key |
...
## `StatementProposalStore` — staging under `.maugham/statements/proposals/` (translation pipeline P5, 2026-09-02)

`Maugham/Stores/StatementProposalStore.swift` stages what the `edition-brief`
and `visual-language` skills draft from Claude Desktop or Claude Code, one
pending slot per key (`edition-brief-<tag>` for a language's brief,
`visual-language` for the single visual-language statement) — craft intent
has no key, because `ProposableStatement` has no case for it. `stage`
supersedes whatever was pending at that key and returns the proposal
re-read from disk, so the in-memory value and the on-disk value can never
drift (the `.iso8601` timestamp round-trips whole-second, matched to
`DesignProposalStore`'s sibling shape); `discard` clears a slot; `validate`
and `glossaryLines` check that a brief's `## Rulings` holds only lines of
the glossary shape (`«term» → «rendering» (note)`) before `stage` accepts
one, and that a visual-language proposal carries no `## Rulings` at all.

**It never writes a statement.** Staging and discarding only ever touch its
own sidecar; the one write into a statement's essay is
`StatementProposalGate.adopt`, a writer's click, described in
`Maugham/Compiler/AREA.md`. Reads carry the `adr-0018-ok` annotation the
same way `DesignProposalStore`'s do, and `MaughamSidecarPath` routes
`.maugham/statements/proposals/` to `.unknownSidecar` deliberately — like
`.maugham/design/`, it is a derived working area with no presenter route of
its own, and losing it costs a pending draft, never a word the writer wrote.

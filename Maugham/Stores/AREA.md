...
- **It opens no `Document` and takes no `lockStatementOpen` of its own** — that gate is over the *opening*, and this only writes a file beside one. `appendToStatement` is where the gate is taken.

## The statement seam — `ProjectStore+Statements.swift` (M1A, 2026-07-31)

**`appendToStatement(_:to:session:)` is where anything OUTSIDE the pane puts words into a statement** *(M1A Task 12 review; it was `PromotionPerformer.append(_:to:)`, and is shared now that visual language's picture ingest reaches it too — read the call sites for who does)*. Its destination is named by **statement**, never by "whatever the pane is showing": its callers suspend — a promotion awaits its snapshot, an ingest awaits a manifest write and a file copy — and the writer can change panes in that window with a keystroke. `session` is the caller's, so the ops carry who wrote them.

**Finding the `Document` to write into is `withStatementDocument(_:session:_:)`'s, and it is the ONE statement open-and-mutate dance** *(S2, 2026-08-09)*: the live `Document` first through `openStatementDocument(id:)`, then the open gate, a re-ask of the registry inside it, then a transient `Document.load`/mutate/awaited-`close`; the lookup-plus-mutate does not suspend, so a pane cannot close its `Document` between them. **It is the seam that takes `lockStatementOpen`** — `appendToStatement` is one of its two callers rather than a taker itself, and the ingest and the promotion are callers of *that*. The second caller is `propagateWikiLinkRename`'s statements loop, and it is why the dance was extracted: a rename that opened its own `Document` on a statement the pane has open would be written back out by the writer's next burst. Who takes the gate is a census — `TripwireGrepTests.statementOpenGateTakers`, which names every member — not a sentence here.

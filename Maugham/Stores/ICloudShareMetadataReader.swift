...
/// Mac `ShareMetadataReading` backed by the OS's iCloud-Drive share resource
...
struct ICloudShareMetadataReader: ShareMetadataReading {
    ...
    func read(for url: URL) -> ShareMetadata? {
        ...
        // currentUserName nil here — Settings falls back to the user's configured
        // collaborator display name for provenance/UI. (Wired in a later task.)
        return ShareMetadata(
            isShared: true,
            isOwner: isOwner,
            canWrite: canWrite,

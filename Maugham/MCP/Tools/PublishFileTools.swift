...
public enum ListPublishFilesTool: MCPTool {
    ...
    "List files under .maugham/publish/ for a project. Returns {files: [{path, size, modified_at}], build_artifacts: [{path, size, modified_at}]} both sorted by path. Top-level files appear in `files`; transient compile outputs under build/ appear in `build_artifacts` (readable via read_publish_file)."
        ...
        if FileManager.default.fileExists(atPath: publishRoot.path),
           let enumerator = FileManager.default.enumerator(
            ...
            while let item = enumerator.nextObject() as? URL {
                ...
                if rel.hasPrefix("build/") {
                    buildArtifacts.append(record)
                } else {
                    files.append(record)
                }
            }

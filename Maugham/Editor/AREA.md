...
2. **Both coordinators answer the window's commands.** ⌘⌥R's review membrane, scene / find-match / paragraph / annotation navigation and the translation membrane are all key-window scoped and all ride `receiverContext(.keyWindow)` — so the intent pane flipped into review chrome, moved its caret when the writer clicked a scene, and went read-only in translation review. `EditorCoordinator.respondsToWindowCommands` gates them at that one funnel, which is why all six are covered rather than the two someone thought of.

...
- **Scoped `maughamScriptDidUpdate` (Channel A, ADR 0017 addendum):** the coordinator's script-did-update post is origin-scoped. Every poster MUST carry the originating project id in `userInfo` under `ScriptUpdateRouting.projectIdKey` (the coordinator stamps `scriptOriginProjectId`, wired by `EditorSurface` from `EditorHost` = `ProjectIdentifier.id(for: store.url)`). Receivers MUST filter via `ScriptUpdateRouting.acceptedScript(from:forProjectId:)` — adopt the script only when the origin matches their own project. This is NOT a key-window guard (a background window's own MCP-driven re-parse must still update its scene navigator). Before scoping, any window flipping to a screenplay piece wrote every other window's `lastParsedScript`, triggering a ~0.8–1s NavigationStack relayout of the bound editor and silently clobbering that window's navigator payload. Regression net: `ScriptUpdateScopingTests`.

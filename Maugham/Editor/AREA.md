...
- `ScreenplayMode.swift` lives at `Maugham/Editor/` (not under `Fountain/`).
  - `applyTypography` takes a `restyleWindow` so callers can `setAttributes` over just a sub-range (the modes still honour it; `WindowedTypographyEquivalenceTests` pins window≡whole-doc equivalence). **NOTE (2026-06-25):** the coordinator no longer windows *per keystroke* — the typing path now defers a single whole-doc repaint to the burst settle (tripwire 9), so `restyleWindow` is effectively only ever nil from the coordinator. The windowing capability is kept on the modes (tested, cheap, and the right primitive if per-keystroke painting ever returns) but `EditorCoordinator` doesn't compute a window anymore. Don't add work *inside* `applyTypography` on either path.
...
9. **Scroll position is hand-governed; a whole-doc `setAttributes` snaps the origin toward the top on a long scrolled doc.** `retokenizeAndStyle`'s full-storage `setAttributes` invalidates all layout, which AppKit then re-lays-out from the top. The mitigation depends on *when* the paint happens:
   - **Typing path (since 2026-06-25, the "settle paint"):** the structural repaint is **deferred** off the keystroke to the trailing edge of the typing burst (`scheduleDeferredRestyle` → `performDeferredRestyle`, ~`restyleSettleDelayMs`=300ms). On the keystroke itself NO paint happens, so there is nothing to snap and **AppKit's native caret-following autoscroll is left to work** — `textDidChange` no longer captures/restores the origin (that old per-keystroke restore was *fighting* the autoscroll: "recoil on the last line" + "caret runs off the bottom mid-document"). **The settle paint is itself WINDOWED to the changed paragraph(s)**, not whole-doc: `shouldChangeTextIn` snapshots the pre-burst text (`burstBaselineText`), and `performDeferredRestyle` character-diffs it against the post-burst text (`changedParagraphWindow`) and restyles only that range — a *local* `setAttributes` doesn't invalidate the whole layout, so it doesn't move scroll at all (no restore needed). Use a CHARACTER diff, not a token diff: plain prose produces no syntax tokens, so the token-window approach falls back to whole-doc and re-introduces the snap (this exact bug shipped in the first cut). The whole-doc + capture/restore branch survives only as a no-baseline defensive fallback. Deferral also kills the emphasis-flicker: a transient invalid syntax state (`*italic *` while editing the end of a run) never gets painted, because the paint waits for rest. **Don't re-introduce a per-keystroke paint, a per-keystroke origin-restore, or a whole-doc settle paint.** (`DeferredRestyleTests`, incl. `test_settleRestyleIsWindowedNotWholeDocument`, is the regression net.)

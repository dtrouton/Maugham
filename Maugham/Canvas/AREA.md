...
- **`sceneRevision` exists in two places on purpose.** `CanvasView`'s `@State` copy keeps its name because `CanvasAccessibilityTests` greps for `.onChange(of: sceneRevision`; the model's copy exists because the inspector mutates the scene from the other column, where no view-local counter is reachable. The view mirrors the model's in one line. **Neither may be keyed on `revision`** (tripwire 30). The model's bump is unconditional — making it conditional on a canvas being mounted would make it *under-count* exactly when a canvas happens to be on screen, so the model would stop being correct on its own and the inspector would miss every undo made while the canvas was up. It is the VIEW's bump that is suppressed on the undo path, through a non-defaulted `rebuildLayouts(bumpsStructuralCounter:)`.
...
**`canvas.json` is derived and deletable; `canvas.md` is content and is not.** Positions, geometry, seeds, z-order **and every region — label, frame, both member sets, bound piece and collapsed flag** — live in `.maugham/canvas.json`; scrap *text* lives in `canvas.md` at the project root. Delete the sidecar by hand and the layout and the regions are gone, the words are not. That split is the point (§8), and it is why the region inspector takes a `CanvasModel` and not a `ProjectStore`: none of this is manifest state.

**The sidecar is at schema 2** (`CanvasSceneDTO.currentSchemaVersion`). `regions` is optional so every schema-1 file 1C-a wrote decodes unchanged rather than throwing on a missing key; in the other direction `CanvasStore.load` refuses a `schemaVersion` above its own and returns an **empty layout with the scraps intact** — so a schema-2 canvas opened by an older build costs the arrangement and never the words. Membership arrays are written `.sorted()`, which is what makes saving an unchanged canvas byte-identical **across processes**: `Set` iteration is deterministic within one run, so a test that saves twice from one value cannot see this — `test_regionMembershipArraysAreWrittenSortedRegardlessOfSetIterationOrder` decodes the DTO and asserts the order instead.

...
**A gap on the record: nothing in production creates an appearance.** `CanvasMembership.addAppearance` has no production caller. The codec round-trips appearances, the renderer draws them as chips hairlined to the real card, and the inspector lists and removes them — and no gesture and no control makes one. That is 1C-a's `CanvasScene.remove` shape (built, exercised, unreachable) repeating one slice later. Whichever slice adds the citing gesture should read this paragraph first, and the delivery-path rule below applies to it.
...
**Regions and `CanvasModel` were on this list and 1C-b built them** — see "Regions, and why membership is never geometry" and "Who owns what" above. **Deleting a scrap was on it too**: ⌫ with something selected is `CanvasView.deleteSelection()`, reached from `CanvasEventNSView.keyDown` — see "Delete" above. What 1C-b leaves genuinely open is the *citing* gesture: appearances are stored, drawn, listed and removable, and nothing creates one.

...
**Nothing creates an appearance yet** — the citing half is stored, drawn, listed and removable, and unreachable. See "What the canvas does not do yet" at the foot of this file; a follow-up builds it in the inspector.
...
- **Before you call a primitive done, grep for its production callers.** A green suite cannot tell a fully-exercised function from a reachable one, and this area has now shipped three unreachable halves — 1C-a's ⌘Z (built, twenty-two tests deep, greyed out in the Edit menu), `CanvasScene.remove` (built, exercised, no key and no menu item), and `CanvasMembership.addAppearance` (stored, drawn, listed, removable, uncreatable). Each was found by a caller count, never by a test. It is one command per function the slice added.
...
**1C-b leaves one gap of the same kind, and it is written here in the same shape so it is as findable as that one was.** `CanvasMembership.addAppearance` has **no production caller**. Appearances are minted by the codec, drawn as chips hairlined back to the real card, listed under "Appears here" in the inspector and removable from there — every part except the one that makes one. The drop gesture calls `join` (home) and nothing else.

- **Writer-visible consequence:** the "Appears here" list is real and persists, and **nothing the writer can do puts anything in it.** It reads permanently empty.
- **Why it is not cosmetic:** *one home, many appearances* is §4.3, and it is what makes this a planning surface rather than a filing one. Without a way to say "this card belongs to that piece *and* is referenced over here", the writer is pushed back into the premature single choice the design exists to avoid.
- **Owner:** a follow-up beside the inspector — an add control symmetric with the existing remove, offering the scraps not already in the region. Inspector-side deliberately: no modifier plumbing through `CanvasEventNSView`, which is the same reason the region-draw gesture was chosen the way it was.
- **How it was found**, because this is the check that would have caught the delete gap a slice earlier: `grep -rn addAppearance Maugham/` — **grep for production callers of every function the slice built.** A green suite cannot see this; the function is fully exercised. Only the caller count can.

The delivery-path rule under "Delete" applies to whatever builds it.

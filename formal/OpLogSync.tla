...
(*   PerDeviceCheckpoints = FALSE  checkpoints are NOT (.maugham/           *)
(*                                 checkpoints.jsonl is one shared file)    *)
(* So the checkpoint configs model the shipping system, and the ones that   *)
(* violate their invariant are describing a live defect, not a hypothetical.*)
(***************************************************************************)
...
CONSTANTS
    ...
    PerDeviceCheckpoints,   \* FALSE = production: one shared
                            \*         .maugham/checkpoints.jsonl
                            \* TRUE  = the proposed ADR 0012 fix applied
...
(*                                                                         *)
(* VIOLATED with PerDeviceCheckpoints = FALSE, which is PRODUCTION.         *)
(* Green with TRUE, which is ADR 0012's pattern applied to                  *)
(* .maugham/checkpoints.jsonl. The pair is the proof that partitioning is   *)
(* the fix and not merely a tidy-up.                                        *)
(***************************************************************************)
CheckpointNoLoss ==

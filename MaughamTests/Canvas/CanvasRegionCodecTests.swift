...
final class CanvasRegionCodecTests: XCTestCase {
    ...
    /// bumps `currentSchemaVersion` has taken since (now 8, 1C-d's `ownedPath`
    /// on top of 1C-c3's `author` on top of 1C-c2b's `contributedToItemID` on
    /// top of 1C-c2a's `boundPieceID` on top of 1C-c2's `promotedItemID`). The
    /// version literal itself is asserted in
    /// `CanvasLineCodecTests.test_theSchemaVersionIsEight`, in
